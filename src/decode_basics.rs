use crate::types::{DecodeError, EthernetHeader, IECData};
...
pub fn decode_boolean(
    ...
    if buffer_index >= buffer.len() {
        return Err(DecodeError::new(
            &format!("Out of bounds for buffer of length {}", buffer.len()),
            buffer_index,
        ));
    }
...
pub fn decode_float_32(
    ...
    if buffer_index + 5 > buffer.len() {
        return Err(DecodeError::new(
            &format!(
                "Attempt to read 5 bytes for f32 exceeds buffer length {}",
                buffer.len()
            ),
            buffer_index,
        ));
...
pub fn decode_tag_length(
    ...
    *tag = buffer[buffer_index];
    ...
    if pos >= buffer.len() {
        return Err(DecodeError::new(
            &format!("Decode tag length: missing length byte "),
            buffer_index,
        ));
    }
...
pub fn decode_iec_data_element(
    buffer: &[u8],
    buffer_index: usize,
) -> Result<(usize, IECData), DecodeError> {
    let mut tag: u8 = 0;
    ...
    match tag {
        ...
        0x83 => {
            ...
            Ok((next_buffer_index, IECData::Boolean(val)))
        }
        ...
        0x85 => match length {
            1 => {
                ...
                Ok((next_buffer_index, IECData::Int8(val)))
            }
            2 => {
                let mut val: i16 = 0;
                let next_buffer_index =
                    decode_integer_16(&mut val, buffer, new_buffer_index, length)?;
                Ok((next_buffer_index, IECData::Int16(val)))
            }
            3..=4 => {
                let mut val: i32 = 0;
                let next_buffer_index =
                    decode_integer_32(&mut val, buffer, new_buffer_index, length)?;
                Ok((next_buffer_index, IECData::Int32(val)))
            }
            5..=8 => {
                let mut val: i64 = 0;
                let next_buffer_index =
                    decode_integer_64(&mut val, buffer, new_buffer_index, length)?;
                Ok((next_buffer_index, IECData::Int64(val)))
            }
            _ => Err(DecodeError::new(
                &format!("oversize signed integer at {}", new_buffer_index),
                buffer_index,
            )),
        },
        ...
        0x86 => match length {
            1 => {
                ...
                Ok((next_buffer_index, IECData::Int8u(val as u8)))
            }
            2 => {
                let mut val: u16 = 0;
                let next_buffer_index =
                    decode_unsigned_16(&mut val, buffer, new_buffer_index, length)?;
                Ok((next_buffer_index, IECData::Int16u(val)))
            }
            3..=4 => {
                let mut val: u32 = 0;
                let next_buffer_index =
                    decode_unsigned_32(&mut val, buffer, new_buffer_index, length)?;
                Ok((next_buffer_index, IECData::Int32u(val)))
            }
            5 => {
                if buffer[new_buffer_index] != 0x00 {
                    return Err(DecodeError::new(
                        "Unsigned integer exceeds supported size",
                        new_buffer_index,
                    ));
                }
                let mut val: u32 = 0;
        ...
        0x87 => match length {
            5 => {
                let mut val: f32 = 0.0;
                let next_buffer_index =
                    decode_float_32(&mut val, buffer, new_buffer_index, length)?;
                Ok((next_buffer_index, IECData::Float32(val)))
            }
            _ => Err(DecodeError::new(
                &format!("Unexpected float size {}", length),
                new_buffer_index,
            )),
        },
        // Visible string
        0x8a => {
            ...
            Ok((next_buffer_index, IECData::VisibleString(val)))
        }
        // MMS string
        0x90 => {
            let mut val: String = "".to_string();
            let next_buffer_index = decode_string(&mut val, buffer, new_buffer_index, length)?;
            Ok((next_buffer_index, IECData::MmsString(val)))
        }
        ...
        0x84 => {
            ...
            Ok((next_buffer_index, IECData::BitString { val, padding }))
        }
        // Array
        0xa1 => {
            ...
            Ok((next_buffer_index, IECData::Array(val)))
        }
        // Structure
        0xa2 => {
            let mut val: Vec<IECData> = vec![];
            ...
            Ok((next_buffer_index, IECData::Structure(val)))
        }
        // Octet string
        0x89 => {
            let mut val: Vec<u8> = vec![0; length];
            let next_buffer_index =
                decode_octet_string(&mut val, buffer, new_buffer_index, length)?;
            Ok((next_buffer_index, IECData::OctetString(val)))
        }
        // UTC time
        0x91 => {
            ...
            Ok((next_buffer_index, IECData::UtcTime(arr)))
        }
        // Unknown or unsupported tag
        _ => Err(DecodeError::new(
            &format!("Unknown data type at {}", new_buffer_index),
            new_buffer_index,
        )),
    }
}
